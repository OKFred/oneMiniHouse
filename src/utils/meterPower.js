let main = (() => {
    ...
    let _deviceAddress = "58";
    let codeRead = "03";
    let codeWrite = "10";
    let energyLength = 18;
    let detailLength = 74;
    let energyByteRead = "04";
    let detailByteRead = "20";

    function addressWriteQuery({ deviceAddress = _deviceAddress, dataWrite = "" } = {}) {
        if (!dataWrite) return false;
        let targetMemory = "0006";
        // let dataWrite = "0007";
        return deviceAddress + codeWrite + targetMemory + dataWrite;
    } //FF0607D0000E; 修改设备地址

    function energyQuery({ deviceAddress = _deviceAddress } = {}) {
        let targetMemory = "4000";
        let byteRead = "0002";
        return deviceAddress + codeRead + targetMemory + byteRead;
    } // 默认（580340000002DD02）读取电能

    function detailQuery({ deviceAddress = _deviceAddress } = {}) {
        let targetMemory = "2000";
        let byteRead = "0010";
        return deviceAddress + codeRead + targetMemory + byteRead;
    } // 默认（580320000010430F）读电压、电流、有功、无功、视在、功率因素、电网频率

    function energyClearQuery({ deviceAddress = _deviceAddress } = {}) {
        let targetMemory = "0002";
        let byteWrite = "0001";
        let byteLength = "02";
        let dataWrite = "0001";
        return deviceAddress + codeWrite + targetMemory + byteWrite + byteLength + dataWrite;
    } //58 10 0002 0001 02 0001(未含cs) 电能清零

    function stopBitChangeQuery({ deviceAddress = _deviceAddress } = {}) {
