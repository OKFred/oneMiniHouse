...
import sensorLight from "/src/utils/sensorLight.js";

const volumeQueryHex = (() => {
  let hex = sensorLight.lightQuery();
  hex += modbus.crc(hex);
  return hex;
})()
  ...
  forms: [
    {
      ...
      formArr: [
        ...
        // { label: "功能码", value: "" },
        // { label: "读取字节数", value: "" },
        { label: "光照", value: "", postfix: '勒克斯', postfixColor: 'grey' },
        // { label: "CRC高位", value: "" },
        // { label: "CRC低位", value: "" },
      ],
    },
...
function dataReader(hex) {
  ...
  if (![sensorLight.deviceAddress].find(str => str === device_address) && !['04'].find(str => str === byte_read)) {
  ...
  if (byte_read === "04") {
    let formName = "主表单";
    if (hex.length < sensorLight.generalLength) {
      props.localObj.currentBuffer = formName;
      // console.log('🚩①长度不足，先放缓存')
      return bufferAdd(hex);
    } else if (hex.length > sensorLight.generalLength) hex = hex.substring(0, sensorLight.generalLength); //长度校验；
    bufferReset();
    if (!crcCheck) return
...
function setForm(formName, hex) {
  ...
  if (formName === '主表单' && hex.length === sensorLight.generalLength) {
    // console.log('✅pass')
    let generalObj = sensorLight.lightReader(hex);
    ...
    formArr.find((obj) => obj.label === "光照").value = lightness;
  }
}
