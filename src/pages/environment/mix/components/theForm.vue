...
function dataReader(hex) {
  ...
  if (![sensorMix.deviceAddress].find(str => str === device_address) /* && !['04'].find(str => str === byte_read) */) {
  ...
  //if (byte_read === "04") {
  let formName = "主表单";
  if (hex.length < sensorMix.generalLength) {
    props.localObj.currentBuffer = formName;
    // console.log('🚩①长度不足，先放缓存')
    return bufferAdd(hex);
  } else if (hex.length > sensorMix.generalLength) hex = hex.substring(0, sensorMix.generalLength); //长度校验；
  bufferReset();
  if (!crcCheck) return
